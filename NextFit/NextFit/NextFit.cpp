...
void NextFit(struct partition *, struct job *, int);
struct job {
	string name;
	int job_size;
...
int main() {
	...
	NextFit(partitions, jobs, partition_job_number);
...
void NextFit(struct partition *partitions, struct job *jobs, int number) {
	int total_waste = 0;
	int nextindex = 0;
	for (int i = 0; i < number; i++)
	{
		for (int j = nextindex; j < number; j++)
		{
...
Job 3: J3 100
Job 4: J4 200
J1 is running in partition 1   waste: 100
J2 is running in partition 3   waste: 100
J3 is running in partition 4   waste: 0
J4 is running in partition 2   waste: 0
